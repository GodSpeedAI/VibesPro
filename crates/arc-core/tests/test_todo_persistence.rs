use arc_core::ui_writer::NullUiWriter;
use arc_core::Agent;
...
async fn create_test_agent_in_dir(temp_dir: &TempDir) -> Agent<NullUiWriter> {
    ...
    let config = arc_config::Config::default();
    let ui_writer = NullUiWriter;

    Agent::new(config, ui_writer).await.unwrap()
}

/// Helper to get todo.arc.md path in temp directory
fn get_todo_path(temp_dir: &TempDir) -> PathBuf {
    temp_dir.path().join("todo.arc.md")
}
...
async fn test_todo_write_creates_file() {
    ...
    // Initially, todo.arc.md should not exist
    assert!(!todo_path.exists(), "todo.arc.md should not exist initially");

    // Create a tool call to write TODO
    let tool_call = arc_core::ToolCall {
    ...
    assert!(
        result.contains("todo.arc.md"),
        "Should mention todo.arc.md: {}",
        result
    );

    // File should now exist
    assert!(todo_path.exists(), "todo.arc.md should exist after write");
...
async fn test_todo_persistence_across_agents() {
    ...
    assert!(
        todo_path.exists(),
        "todo.arc.md should persist after agent drops"
    );
...
async fn test_todo_update_preserves_file() {
    ...
    // Write initial TODO
    let write_call = arc_core::ToolCall {
        tool: "todo_write".to_string(),
        args: serde_json::json!({
            "content": "- [ ] Task 1\n- [ ] Task 2"
        }),
    ...
    // Update TODO
    let update_call = arc_core::ToolCall {
        tool: "todo_write".to_string(),
        args: serde_json::json!({
            "content": "- [x] Task 1\n- [ ] Task 2\n- [ ] Task 3"
        }),
...
async fn test_todo_agent_initialization_loads_file() {
    ...
    // Pre-create todo.arc.md before agent initialization
...
async fn test_todo_handles_unicode_content() {
    ...
    let read_call = arc_core::ToolCall {
        tool: "todo_read".to_string(),
        args: serde_json::json!({}),
    };
...
async fn test_todo_empty_content_creates_empty_file() {
    ...
    assert!(todo_path.exists(), "Empty todo.arc.md should create file");
    let content = fs::read_to_string(&todo_path).unwrap();
    assert_eq!(content, "");
}
