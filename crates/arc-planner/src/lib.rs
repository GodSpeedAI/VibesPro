//! arc-planner: Planning mode and fast-discovery planner for ARC AI coding agent
//!
//! This crate provides:
...
fn write_code_report(report: &str) -> Result<()> {
    ...
    let logs_dir = if let Ok(workspace_path) = std::env::var("ARC_WORKSPACE_PATH") {
...
fn write_discovery_commands(commands: &[String]) -> Result<()> {
    ...
    file.write_all(b"# Generated by arc-planner\n\n")?;
