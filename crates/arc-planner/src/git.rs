...
pub fn stage_files(codepath: &Path, plan_dir: &Path) -> Result<StagingResult> {
    ...
    if !add_plan_output.status.success() {
        ...
        if !stderr.contains("did not match any files") {
            anyhow::bail!("Failed to stage arc-plan directory: {}", stderr);
        }
    }
...
/// Re-stage the arc-plan directory to capture any changes made after initial staging.
///
...
pub fn stage_plan_dir(codepath: &Path, plan_dir: &Path) -> Result<()> {
    ...
    let add_output = Command::new("git")
        ...
        .current_dir(codepath)
        .output()
        .context("Failed to re-stage arc-plan directory")?;

    if !add_output.status.success() {
        let stderr = String::from_utf8_lossy(&add_output.stderr);
        anyhow::bail!("Failed to re-stage arc-plan directory: {}", stderr);
    }
