...
impl PlannerConfig {
    /// Get the arc-plan directory path
    pub fn plan_dir(&self) -> PathBuf {
        self.codepath.join("arc-plan")
    }
    ...
    /// Get the path to todo.arc.md
    pub fn todo_path(&self) -> PathBuf {
        self.plan_dir().join("todo.arc.md")
    }
...
pub fn initialize_plan_dir(config: &PlannerConfig) -> Result<()> {
    ...
    if !plan_dir.exists() {
        fs::create_dir_all(&plan_dir)
            .context("Failed to create arc-plan directory")?;
...
pub fn check_git_status(config: &PlannerConfig) -> Result<()> {
    ...
    let ignore_pattern = "arc-plan/new_requirements.md";
...
pub fn prompt_for_new_requirements(config: &PlannerConfig) -> Result<PlannerState> {
    ...
    if todo_path.exists() {
        fs::remove_file(&todo_path)
            .context("Failed to delete old todo.arc.md")?;
    }

    // Display prompt
    let prompt = r#"I will help you refine the current requirements of your project.
    Please write or edit your requirements in `{codepath}/arc-plan/new_requirements.md`.
    ...
    if !new_req_path.exists() {
        let error_msg = "File not found: {path}/arc-plan/new_requirements.md"
...
pub fn prompt_for_approval(config: &PlannerConfig) -> Result<ApprovalChoice> {
    let prompt = r#"The LLM has updated `{codepath}/arc-plan/new_requirements.md`.
...
pub fn read_todo(config: &PlannerConfig) -> Result<Option<String>> {
    ...
    if path.exists() {
        Ok(Some(fs::read_to_string(&path)
            .context("Failed to read todo.arc.md")?))
    } else {
        Ok(None)
    }
}
...
pub fn complete_implementation(config: &PlannerConfig) -> Result<(String, String)> {
    ...
    if todo_path.exists() {
        fs::rename(&todo_path, &completed_todo)
            .context("Failed to rename todo.arc.md")?;
...
pub fn stage_and_commit(
    ...
    // Re-stage arc-plan directory to include the GIT COMMIT entry we just wrote
...
pub async fn run_coach_player_loop(
    ...
    arc_config: &arc_config::Config,
    requirements_content: &str,
) -> Result<()> {
    use arc_core::project::Project;
    use arc_core::retry::{execute_with_retry, RetryConfig, RetryResult};
    use arc_core::feedback_extraction::{extract_coach_feedback, FeedbackExtractionConfig};
    use arc_core::Agent;
    ...
    std::env::set_var("ARC_TODO_PATH", planner_config.todo_path().display().to_string());
    ...
    while turn <= max_turns {
        ...
        let player_config = arc_config.for_player()?;
        ...
        let player_prompt = if coach_feedback.is_empty() || turn == 1 {
            ...
            format!(
                "You are ARC in implementation mode. Address the following coach feedback:\n\n{}\n\nOriginal requirements:\n{}\n\nFix the issues mentioned above.",
                coach_feedback,
                requirements_content
            )
        };
        ...
        let coach_config = arc_config.for_coach()?;
...
pub async fn run_planning_mode(
    ...
    // Set ARC_WORKSPACE_PATH environment variable EARLY for all logging
    std::env::set_var("ARC_WORKSPACE_PATH", workspace_dir.display().to_string());
    ...
    loop {
        state = match state {
            ...
            PlannerState::RefineRequirements => {
                ...
                let arc_config = arc_config::Config::load(config.config_path.as_deref())?;
...
mod tests {
    ...
    fn test_planner_config_paths() {
        ...
        assert_eq!(config.plan_dir(), PathBuf::from("/test/project/arc-plan"));
        assert_eq!(config.new_requirements_path(), PathBuf::from("/test/project/arc-plan/new_requirements.md"));
        assert_eq!(config.current_requirements_path(), PathBuf::from("/test/project/arc-plan/current_requirements.md"));
        assert_eq!(config.todo_path(), PathBuf::from("/test/project/arc-plan/todo.arc.md"));
    }
