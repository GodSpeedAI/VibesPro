...
fn setup_test_git_repo() -> Result<TempDir> {
    ...
    // Create arc-plan directory
    let plan_dir = repo_path.join("arc-plan");
...
fn test_history_written_before_commit_on_empty_staging() {
    ...
    use arc_planner::planner::PlannerConfig;
    use arc_planner::history;
    ...
    let commit_result = arc_planner::git::commit(&config.codepath, summary, "Test description");
...
fn test_history_written_before_successful_commit() {
    ...
    // Now make the commit
    let commit_result = arc_planner::git::commit(repo_path, summary, "Test description");
...
fn test_multiple_history_entries_with_failures() {
    ...
    let _ = arc_planner::git::commit(repo_path, "Commit 1 - will fail", "Desc 1");
    ...
    let _ = arc_planner::git::commit(repo_path, "Commit 2 - will succeed", "Desc 2");

    // Third entry - will fail (nothing staged)
    history::write_git_commit(&plan_dir, "Commit 3 - will fail").expect("Failed to write history");
    let _ = arc_planner::git::commit(repo_path, "Commit 3 - will fail", "Desc 3");
...
fn test_history_entry_format() {
    let temp_dir = setup_test_git_repo().expect("Failed to setup test repo");
    let plan_dir = temp_dir.path().join("arc-plan");
...
fn test_stage_plan_dir_captures_history_changes() {
    ...
    assert!(status_after_str.contains("arc-plan/planner_history.txt"),
