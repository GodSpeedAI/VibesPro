...
use arc_ensembles::{FlockConfig, FlockMode};
use std::fs;
...
fn create_test_config(temp_dir: &TempDir) -> PathBuf {
    let config_path = temp_dir.path().join(".arc.toml");
...
fn test_flock_config_builder() {
    ...
    .with_max_turns(15)
    .with_arc_binary(PathBuf::from("/custom/g3"));

    assert_eq!(config.num_segments, 2);
    assert_eq!(config.max_turns, 15);
    assert_eq!(config.arc_binary, Some(PathBuf::from("/custom/g3")));
}
