...
pub struct FlockConfig {
    ...
    /// ARC configuration to use for agents
    pub arc_config: Config,

    /// Path to arc binary (defaults to current executable)
    pub arc_binary: Option<PathBuf>,
}
...
impl FlockConfig {
    ...
    pub fn new(
        ...
        // Load default config
        let arc_config = Config::load(None).or_else(|_| {
            // If no config file exists, return an error with helpful message
            anyhow::bail!("No ARC configuration found. Please create a .arc.toml file.")
        })?;
    ...
    pub fn new_with_config(
        ...
        let arc_config = Config::load(config_path)?;
    ...
    /// Set custom arc binary path
    pub fn with_arc_binary(mut self, binary: PathBuf) -> Self {
        self.arc_binary = Some(binary);
        self
    }

    /// Set custom arc config
...
impl FlockMode {
    ...
    async fn partition_requirements(&mut self) -> Result<Vec<String>> {
        ...
        // Get arc binary path
        let arc_binary = self.get_arc_binary()?;

        // Run arc in single-shot mode to partition requirements
        println!("   Analyzing requirements and creating partitions...");
        let output = Command::new(&arc_binary)
            .arg("--workspace")
            ...
            .output()
            .await
            .context("Failed to run arc for partitioning")?;
    ...
    /// Get the arc binary path
    fn get_arc_binary(&self) -> Result<PathBuf> {
        if let Some(ref binary) = self.config.arc_binary {
            Ok(binary.clone())
        } else {
...
async fn run_segment(
    ...
    // Run arc in autonomous mode with segment-requirements.md
    let mut child = Command::new(&arc_binary)
        .arg("--workspace")
        .arg(&segment_dir)
        .arg("--autonomous")
        .arg("--max-turns")
        ...
        .stderr(Stdio::piped())
        .spawn()
        .context("Failed to spawn arc process")?;
    ...
    let status = child
        .wait()
        .await
        .context("Failed to wait for arc process")?;
...
mod tests {
  ...
  },
  {
    ...
    "requirements": "Add functionality that will summarise the entire message history every time it is sent to LLM. Put it in the logs directory the same as the workspace logs for message history. Call it \"context_window_<suffix>\" where the suffix is the same name as will be used for logging the message history, for example \"arc_session_you_are_arc_in_coach_f79be2a46ac40c35.json\". Look at the code that generates that file name in ARC and use the same code. This file name changes every time and new agent is created, so follow the same pattern with the context window summary. Whenever the file name changes, update a symlink called \"current_context_window\" to that new file. Every time the message history is sent to the LLM, rewrite the entire file. Each message should only take up one line. The format is: date&time, estimated number of tokens of the entire message (use the token estimator code in ARC, write it in a compact way for example 1K, 2M, 100b, 200K, colour code it graded from bright green to dark red where 200b is bright green and 50K is dark red), message ID, role (e.g. \"user\", \"assistant\"), the first hundred characters of \"content\".",
...
[1A[2K│ [2m- Integrate with existing ARC logging infrastructure[0m
[1A[2K│ [2m[0m
...
- Integrate with existing ARC logging infrastructure
