...
window.handleKill = async function (id) {
  ...
  button.innerHTML =
    '<span class="spinner" style="width: 1rem; height: 1rem; border-width: 2px; display: inline-block; vertical-align: middle;"></span> Terminating...';
...
window.handleRestart = async function (id) {
  ...
  button.disabled = true;
  button.innerHTML =
    '<span class="spinner" style="width: 1rem; height: 1rem; border-width: 2px; display: inline-block; vertical-align: middle;"></span> Restarting...';
  ...
  try {
    ...
    button.innerHTML =
      '<span class="spinner" style="width: 1rem; height: 1rem; border-width: 2px; display: inline-block; vertical-align: middle;"></span> Starting...';
...
const modal = {
    ...
    const models = {
      databricks: [
        { value: 'databricks-claude-sonnet-4-5', label: 'databricks-claude-sonnet-4-5' },
        {
          ...
          label: 'databricks-meta-llama-3-1-405b-instruct',
        },
      ],
      anthropic: [
      ...
      local: [{ value: 'local-model', label: 'Local Model' }],
    };
  ...
  async handleLaunch() {
    ...
    try {
      ...
      submitBtn.innerHTML =
        '<span class="spinner" style="width: 1rem; height: 1rem; border-width: 2px; display: inline-block; vertical-align: middle;"></span> Starting g3 instance...';

      const response = await api.launchInstance(data);

      // Show intermediate state
      submitBtn.innerHTML =
        '<span class="spinner" style="width: 1rem; height: 1rem; border-width: 2px; display: inline-block; vertical-align: middle;"></span> Waiting for process...';
      ...
      // Save state
      state.updateLaunchDefaults(data.workspace, data.provider, data.model, data.g3_binary_path);
      ...
      errorDiv.style.cssText =
        'background: #fee; border: 1px solid #fcc; color: #c33; padding: 1rem; margin: 1rem 0; border-radius: 0.5rem;';
      ...
      if (error.message && error.message.includes('400')) {
        errorMessage =
          'Invalid configuration. Please check that the g3 binary path exists and is executable, and that the workspace directory is valid.';
