...
function InstancePanel({ instance, onClick, onKill, onRestart }) {
  ...
  return (
    <div onClick={onClick} className="hero-card p-6 cursor-pointer">
      <div className="flex justify-between items-start mb-4">
        ...
        <div className="flex gap-2">
          {inst.status === 'running' && (
            <button onClick={handleKill} className="hero-button hero-button-danger text-sm">
              Kill
            </button>
          )}
          {inst.status === 'terminated' && (
            <button onClick={handleRestart} className="hero-button hero-button-secondary text-sm">
              Restart
            </button>
          )}
        </div>
      ...
      <ProgressBar instanceType={inst.instance_type} durationSecs={stats.duration_secs} />
      ...
      <div className="grid grid-cols-3 gap-4 mt-4">
        ...
        <div>
          ...
          <div className="text-lg font-semibold text-gray-900 dark:text-white">{stats.errors}</div>
        </div>
      </div>
      ...
      <div className="mt-2 text-xs text-gray-500 dark:text-gray-500">{inst.workspace}</div>
    </div>
  );
}
