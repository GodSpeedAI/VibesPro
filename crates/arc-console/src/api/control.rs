...
pub async fn restart_instance(
    ...
    let new_pid = controller
        .launch_arc(
            ...
            &params.prompt,
            params.autonomous,
            params.arc_binary_path.as_deref(),
        )
        .map_err(|e| {
...
pub async fn launch_instance(
    ...
    info!("Launching new arc instance: {:?}", request);

    // Validate binary path if provided
    if let Some(ref binary_path) = request.arc_binary_path {
        ...
        if !path.exists() {
            error!("ARC binary not found: {}", binary_path);
            return Err((
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({
                    "error": "ARC binary not found",
                    "message": format!("The specified arc binary does not exist: {}", binary_path)
                })),
            ));
        }
        ...
        #[cfg(unix)]
        {
            ...
            if let Ok(metadata) = std::fs::metadata(path) {
                if metadata.permissions().mode() & 0o111 == 0 {
                    error!("ARC binary is not executable: {}", binary_path);
                    return Err((
                        StatusCode::BAD_REQUEST,
                        Json(serde_json::json!({
                            "error": "ARC binary is not executable",
                            "message": format!("The specified arc binary is not executable: {}", binary_path)
                        })),
                    ));
                }
            }
    ...
    let arc_binary_path = request.arc_binary_path.as_deref();

    let mut controller = controller.lock().await;

    match controller.launch_arc(
        workspace,
        &request.provider,
        &request.model,
        &request.prompt,
        ...
        Ok(pid) => {
            ...
            info!("Successfully launched arc instance with PID {}", pid);
            Ok(Json(LaunchResponse {
                id,
                status: "starting".to_string(),
            }))
        ...
        Err(e) => {
            error!("Failed to launch arc instance: {}", e);
            Err((
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(serde_json::json!({
