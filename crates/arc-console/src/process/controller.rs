...
impl ProcessController {
    ...
    pub fn launch_arc(
        ...
        prompt: &str,
        autonomous: bool,
        arc_binary_path: Option<&str>,
    ) -> Result<u32> {
        let binary = arc_binary_path.unwrap_or("arc");
            ...
            cmd.pre_exec(|| {
                ...
                match libc::fork() {
                    ...
                    0 => {
                        ...
                        libc::setsid();
                        // Continue execution (this becomes the actual arc process)
                    }
                    _ => {
        ...
        let mut child = cmd.spawn().context("Failed to spawn arc process")?;
        ...
        // The actual arc process is now running as orphan
        // We need to scan for it by matching workspace and recent start time
        info!(
            "Scanning for newly launched arc process in workspace: {}",
            workspace
        );
        ...
        for (pid, process) in self.system.processes() {
            ...
            // Check if this is a arc process
            let is_arc = process.name().contains("arc") || cmd_str.contains("arc");
            if !is_arc {
                continue;
            }
        ...
        info!("Launched arc process with PID {}", pid);
        ...
        let params = LaunchParams {
            ...
            autonomous,
            arc_binary_path: arc_binary_path.map(|s| s.to_string()),
        };
    ...
    fn parse_launch_params_from_cmd(&self, cmd: &[String]) -> Option<LaunchParams> {
        ...
        if !cmd.is_empty() {
            let first = &cmd[0];
            if first.contains("arc") && !first.contains("cargo") {
                arc_binary_path = Some(first.clone());
            }
        }
