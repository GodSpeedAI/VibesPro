...
fn main() {
    ...
    println!("Looking for arc processes...");
    ...
    for (pid, process) in sys.processes() {
        ...
        if cmd_str.contains("arc") {
            println!("\nFound potential arc process:");
            println!("  PID: {}", pid);
            ...
            let is_arc_binary = cmd.get(0).map(|s| s.ends_with("arc")).unwrap_or(false);
            let is_cargo_run = cmd.get(0).map(|s| s.contains("cargo")).unwrap_or(false)
                && cmd.iter().any(|s| s == "run" || s.contains("arc"));

            println!("  is_arc_binary: {}", is_arc_binary);
