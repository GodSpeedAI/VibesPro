...
fn extract_coach_feedback_from_logs(
    coach_result: &arc_core::TaskResult,
    coach_agent: &arc_core::Agent<ConsoleUiWriter>,
    output: &SimpleOutput,
) -> Result<String> {
    ...
    let log_file_path = logs_dir.join(format!("arc_session_{}.json", session_id));
...
use arc_core::{project::Project, ui_writer::UiWriter, Agent, DiscoveryOptions};
...
use arc_core::error_handling::{classify_error, ErrorType, RecoverableError};
mod simple_output;
mod ui_writer_impl;
...
pub async fn run() -> Result<()> {
    ...
    if cli.planning {
        ...
        // The expand_codepath function in arc_planner handles tilde expansion
        let codepath = cli.codepath.clone();
        return arc_planner::run_planning_mode(
            codepath,
            cli.workspace.clone(),
            cli.no_git,
            cli.config.as_deref(),
    ...
    if !cli.machine {
        ...
        let filter = if cli.verbose {
            EnvFilter::from_default_env()
                ...
                .add_directive("arc_core=debug".parse().unwrap())
                .add_directive("arc_cli=debug".parse().unwrap())
                .add_directive("arc_execution=debug".parse().unwrap())
                .add_directive("arc_providers=debug".parse().unwrap())
        } else {
            EnvFilter::from_default_env()
                .add_directive(format!("{}=info", env!("CARGO_PKG_NAME")).parse().unwrap())
                .add_directive("arc_core=info".parse().unwrap())
                .add_directive("arc_cli=info".parse().unwrap())
                .add_directive("arc_execution=info".parse().unwrap())
                .add_directive("arc_providers=info".parse().unwrap())
    ...
    let workspace_dir = if let Some(ws) = &cli.workspace {
        ...
        // For autonomous mode, use ARC_WORKSPACE env var or default
...
async fn run_flock_mode(
    ...
    output.print("");
    output.print("🦅 ARC FLOCK MODE - Parallel Multi-Agent Development");
    output.print("");
    ...
    let config = arc_ensembles::FlockConfig::new(project_dir, flock_workspace, num_segments)?
        .with_max_turns(max_turns);

    // Create and run flock mode
    let mut flock = arc_ensembles::FlockMode::new(config)?;
...
async fn run_accumulative_mode(
    ...
    output.print("");
    output.print("arc programming agent - autonomous mode");
    ...
    let history_file = dirs::home_dir().map(|mut path| {
        path.push(".arc_accumulative_history");
        path
    });
...
async fn run_autonomous_machine(
    ...
    let task = format!(
        "You are ARC in implementation mode. Read and implement the following requirements:\n\n{}\n\nImplement this step by step, creating all necessary files and code.",
        requirements
    );
...
fn read_project_readme(workspace_dir: &Path) -> Option<String> {
    // Check if we're in a project directory (contains .arc or .git)
    let is_project_dir = workspace_dir.join(".arc").exists() || workspace_dir.join(".git").exists();
...
async fn run_interactive<W: UiWriter>(
    ...
    output.print("");
    output.print("arc programming agent");
    ...
    loop {
        ...
        let prompt = if in_multiline { "... > " } else { "arc> " };
...
/// Uses ARC_WORKSPACE environment variable or defaults to ~/tmp/workspace
fn setup_workspace_directory(machine_mode: bool) -> Result<PathBuf> {
    let workspace_dir = if let Ok(env_workspace) = std::env::var("ARC_WORKSPACE") {
...
async fn run_autonomous(
    ...
    if !project.has_requirements() {
        ...
        output.print("      arc --autonomous --requirements \"Your requirements here\"");
        output.print("");
    ...
    let (discovery_messages, discovery_working_dir): (Vec<arc_providers::Message>, Option<String>) =
        if let Some(ref codebase_path) = codebase_fast_start {
            ...
            match agent.get_provider() {
                Ok(provider) => {
                    ...
                    let status_callback: arc_planner::StatusCallback = Box::new(move |msg: &str| {
                        output_clone.print(msg);
                    });
                    match arc_planner::get_initial_discovery_messages(
                        &path_str,
                        Some(&requirements),
                        provider,
    ...
    loop {
        ...
        arc_core::fixed_filter_json::reset_fixed_json_tool_state();
        ...
        let coach_prompt = format!(
            "You are ARC in coach mode. Your role is to critique and review implementations against requirements and provide concise, actionable feedback.
