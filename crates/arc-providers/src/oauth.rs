...
fn get_base_path() -> PathBuf {
    ...
    // macOS/Linux: ~/.config/arc/databricks/oauth
...
impl OAuthFlow {
    ...
    async fn execute(&self) -> Result<TokenData> {
                        ...
                        if received_state == state {
                            if let Some(sender) = tx.lock().await.take() {
                                if sender.send(code).is_ok() {
                                    return Html(
                                        "<h2>ARC Authentication Success</h2><p>You can close this window and return to your terminal.</p>",
                                    );
                                }
                            }
        ...
        if std::env::var("ARC_RETRO_MODE").is_err() {
