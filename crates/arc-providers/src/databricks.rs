//! Databricks LLM provider implementation for the arc-providers crate.
//!
...
//!
//! ```rust,no_run
//! use arc_providers::{DatabricksProvider, LLMProvider, CompletionRequest, Message, MessageRole};
//!
//! #[tokio::main]
    ...
    async fn complete(&self, request: CompletionRequest) -> Result<CompletionResponse> {
        ...
        if let Some(first_choice) = databricks_response.choices.first() {
            if let Some(tool_calls) = &first_choice.message.tool_calls {
                ...
                // For now, we'll return the content as-is since arc handles tool calls via streaming
